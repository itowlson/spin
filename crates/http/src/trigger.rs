...
const WASI_HTTP_EXPORT_0_3_0_RC_02: &str = "wasi:http/handler@0.3.0-rc-2026-03-15";
...
impl<T, S: HandlerState<StoreData = T>> HandlerType<S> {
    ...
    pub fn validate_executor(&self, executor: &Option<HttpExecutorType>) -> anyhow::Result<()> {
        match (self, executor) {
            ...
            (HandlerType::Wasi0_3(_), Some(_) | None) => {
                anyhow::bail!(
                    "`{WASI_HTTP_EXPORT_0_3_0_RC_02}` is currently unstable and will be \
            ...
            (handler_type, Some(HttpExecutorType::Wasip3Unstable)) => {
                anyhow::bail!(
                    ...
                    exports `{WASI_HTTP_EXPORT_0_3_0_RC_02}` but found a \
    ...
    fn name(&self) -> &str {
        match self {
            ...
            HandlerType::Wasi0_3(_) => WASI_HTTP_EXPORT_0_3_0_RC_02,
