        ...
        for trigger in configured_app.app().triggers_with_type(trigger_type.unwrap()) {  // todo no unwrap
            if let Ok(component) = trigger.component() {
                ...
                let instance_pre = component_loader
                    .load_instance_pre(&self.core_engine, &component, &complications, &complicator)
                    .await?;
...
pub trait ComponentLoader<T: RuntimeFactors, U>: Sync {
    ...
    async fn load_instance_pre(
        ...
        let component = self.load_component(engine.as_ref(), component, complications, complicator).await?;
